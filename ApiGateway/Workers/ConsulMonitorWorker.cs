...
namespace ApiGateway.Workers
{
    public class ConsulMonitorWorker : BackgroundService
    {
        ...
        public ConsulMonitorWorker(IConsulClient consulClient
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                await Task.Delay(TimeSpan.FromSeconds(DEFAULT_CONSUL_POLL_INTERVAL_SECONDS)
                                    , stoppingToken);
            }
        }
        ...
        private async Task<List<ClusterConfig>> ExtractClusters(QueryResult<Dictionary<string, AgentService>> serviceResult)
        {
            ...
            foreach (var (key, svc) in serviceMapping)
            {
                ...
                if (clusterErrs.Any())
                {
                    ...
                    clusterErrs.ToList().ForEach(err =>
